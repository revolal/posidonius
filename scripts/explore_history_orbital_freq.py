...
if __name__ == "__main__":
    ...
    for key in planets_keys:
        ...
        Pseudosynchro_ratio = (1. +(15./2.)*np.power(planet_data['eccentricity'],2) +(45./8.)*np.power(planet_data['eccentricity'],4) +(5./16.)*np.power(planet_data['eccentricity'],6) ) / ( ( 1. +3.*np.power(planet_data['eccentricity'],3) +(3./8.)*np.power(planet_data['eccentricity'],4) )*np.power( (1. -np.power(planet_data['eccentricity'],2)), 3./2.) ) 
    ...
    output_figure_filename = os.path.join(output_figure_dirname, os.path.splitext(os.path.basename(filename))[0] + "_1e_orbital_frequ.png")
